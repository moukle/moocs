...
student = (name = "Moi", kerberos_id = "moukle")
...
begin
	...
	function quantize(color::AbstractRGB)
		return RGB(mapc(c -> quantize(c), color))
	end
...
# ╔═╡ dcc74988-f76e-11ea-3f8f-3d716e6b2460
quantize(RGB(0.123123, 0.9999, 0.8888))
...
begin
	function noisify(x::Number, s)
		noise = (2 * rand() - 1) * s
		return clamp(x + noise, -1, 1)
	end
	
	function noisify(color::AbstractRGB, s)
		return RGB(mapc(c -> noisify(c, s), color))
	end
...
function blur_1D(v, l)
	new_v = zeros(size(v, 1))
	extended_v = [extend(v, i) for i in -l:size(v,1)+l]
	for i in l+1:size(v,1)+l
		window = mean(extended_v[i-l:i+l])
		new_v[i-l] = window
	end
	return new_v
end
...
# ╔═╡ e51b4b02-f77d-11ea-3f23-7571ed099c7f
@bind blur_window Slider(0:1:10, show_value=true)

# ╔═╡ ca1ac5f4-ee1c-11ea-3d00-ff5268866f87
colored_line(blur_1D(v, blur_window))
...
# ╔═╡ 507424f2-f781-11ea-3ab6-99464742d1ef
begin
	a = [1 2 3]
	b = [4 5 6]
...
function gaussian_kernel(n, σ=1)
	kernel = [1 / √(2*π*σ) * exp(-x^2 / 2*σ^2) for x in -n:n]
...
# ╔═╡ 396f3d9c-f79e-11ea-2a4f-f324ef491dc0
@bind gaussian_kernel_size_1D Slider(0:1:10, show_value=true)
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	k_mid_r = (k_rows-1) ÷ 2
	k_mid_c = (k_columns-1) ÷ 2
	
	M_ = zeros(typeof(M[1,1]), size(M))
	
	for i = 1:m_rows, j = 1:m_columns
		mm = [extend_mat(M, ii, jj) for 
				ii=i-k_mid_r:i+k_mid_r,
				jj=j-k_mid_c:j+k_mid_c]
		M_[i,j] = clamp01(sum(mm .* K))
	end
	
	return M_
end

# ╔═╡ 51f29d70-f7ab-11ea-39d2-af98b71497da


# ╔═╡ e6da0122-f7aa-11ea-0e34-a51ff4d12721
...
# ╔═╡ 9a205f56-f7a6-11ea-177e-6dc4c49f2b01
function gaussian_kernel_2d(n, m, σ=1)
	n, m = (n-1) ÷ 2, (m-1) ÷ 2
	
	kernel = [1 / √(2*π*σ) * exp(-(x^2 + y^2) / 2*σ^2) 
		for x =-n:n, y=-m:m]
	return kernel ./ sum(kernel)
end

# ╔═╡ 857599ee-f7a7-11ea-3fbd-1d3f3184ccc0
gaussian_kernel_2d(3,3)

# ╔═╡ aad67fd0-ee15-11ea-00d4-274ec3cda3a3
function with_gaussian_blur(image)
	kernel = gaussian_kernel_2d(3,3)
...
# ╔═╡ 0bba1b86-f7a8-11ea-116b-671502ff0269
philip

# ╔═╡ 8a3505c4-f7a6-11ea-1f34-4587520fd05e
...
# ╔═╡ 154553a2-f7a8-11ea-0535-335195c21d02
function sobel_kernel()
	k_x = kron([1 2  1]', [1 0 -1])
	k_y = kron([1 0 -1]', [1 2  1])
	return reshape.([k_x, k_y], 3,3)
end

# ╔═╡ 9eeb876c-ee15-11ea-1794-d3ea79f47b75
function with_sobel_edge_detect(image)
	kx, ky = sobel_kernel()
	img = convolve_image(image, kx) + convolve_image(image, ky)
	#dump(img)
end

# ╔═╡ 4bfddf2e-f7ab-11ea-00f2-5333e856e739


# ╔═╡ 7332fb1c-f7a9-11ea-2e00-b14c417a685c
