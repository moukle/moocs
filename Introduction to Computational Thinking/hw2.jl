...
function remove_in_each_row_no_vcat(img, column_numbers)
	...
	for (i, j) in enumerate(column_numbers)
		...
		# to avoid using `vcat`.
		img′[i, :] .= deleteat!(img[i, :], j)
		#[img[i, 1:j-1]; img[i, j+1:end]]
	end
	img′
end
...
function greedy_seam(energies, starting_pixel::Int)
	seam = zeros(Int64, size(energies,1))
	seam[1] = starting_pixel
	
	last_pixel = starting_pixel
	for i in 2:size(energies, 1)
		# compute neighbors
		left = clamp(last_pixel-1, 1, size(energies,2))
		right = clamp(last_pixel+1, 1, size(energies,2))
		next_pixel = argmin(energies[i, left:right])

		if right - left < 2 && left == 1
			next_pixel -= 1
		else
			next_pixel -= 2 # make it relative ... (left = -1, mid=0, right=1)
		end
		

		# update seam and last pixel for next iteration
		seam[i] = clamp(last_pixel + next_pixel, 1, size(energies,2))
		last_pixel = seam[i]
	end
...
# ╔═╡ 7148c030-f9fb-11ea-37df-8780ed3a5dd7
greedy_seam(greedy_test, greedy_starting_pixel)
